...
export function useEditForm({ form, obj, props, emit, visible, lnglat }: { form: Ref<ToiletForm>, obj: ToiletForm, props: any, emit: any, visible: Ref<boolean>, lnglat: ModelRef<Lnglat> }) {
  ...
  const rules = ref<FormRules<ToiletForm>>({
    name: [
      { required: true, message: '请输入厕所名称', trigger: 'blur' },
    ],
    introduce: [
      { required: true, message: '请输入厕所介绍, 尽可能多点', trigger: 'blur' },
    ],
    lnglat: [
    ...
    await formEl.validate(async (valid) => {
      if (valid) {
        ...
        if (form.value.attraction) {
          await lc.update('Toilet', form.value.objectId, uploadForm)
        } else {
          const attraction = JSON.parse(localStorage.getItem('attraction') as string)
          await lc.create('Toilet', {
            ...uploadForm,
