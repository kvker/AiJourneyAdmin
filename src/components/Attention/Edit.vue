...
import { useEditForm } from '@/composables/attention/editForm'
...
const form = ref<AttentionForm>({ ...obj })
...
<template>
  <dialog ref="editDialog" class="modal" @close="onCloseEditDialog">
    <div class="modal-box">
      <h3 class="font-bold text-lg mb-2">事项信息</h3>
      <form @submit.prevent="onSubmit">
        <input class="input w-full max-w-xs mb-2" v-model.trim="form.name" placeholder="事项名字" required maxlength="100"
          minlength="2" />
        <div class="w-full mb-2">
          <textarea class="textarea w-full mb-2" v-model="form.introduce" placeholder="事项介绍" required minlength="2"
        ...
        <button class="btn btn-primary" type="submit">{{ form.attractionId ? '更新' : '创建' }}事项</button>
      </form>
