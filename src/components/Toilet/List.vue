...
const searchParams = inject('searchParams') as Ref<ToiletSearchParams>
...
async function getList(params = searchParams.value) {
  ...
  const ret = await lc.read('Toilet', q => {
...
async function getCount(params = searchParams.value) {
  const ret = await lc.count('Toilet', q => {
    if (params.name) {
      q.equalTo('name', params.name)
    }
  })
...
function doReviewLnglat(data: Toilet) {
  console.log('doReviewLnglat')
  console.log(data.lnglat)
  emit('lnglat', data.lnglat)
}

function doEdit(data: Toilet, index: number) {
  console.log('doEdit')
  console.log(data, index)
  emit('edit', data)
}

function doDelete(data: Toilet, index: number) {
  ...
  ElMessageBox.confirm(
    ...
    .then(async () => {
      await lc.delete('Toilet', data.objectId)
...
<template>
  <div class="flex-1 flex flex-col justify-around">
    <el-table :data="tableData" class=" w-full" style="height: 560px;" :row-class-name="tableRowClassName">
      <el-table-column prop="name" label="厕所名" width="180" />
